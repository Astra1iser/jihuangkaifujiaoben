...
#以下my_api_link为原作者南风颂脚本地址
my_api_link="https://wqlin.com/tools/dst.php"
feed_back_link="6号楼315宿舍"
#以下script_url为原作者南风颂脚本地址下载
...
Menu(){
  ...
  do
    Get_game_beta_str
    echo -e "\e[33m============欢迎使用============\e[0m"
    echo -e "\e[33m作者：315白给王 千寻 白给地址==>${feed_back_link}\e[0m"
    echo -e "\e[33m       拿到脚本你说了算\e[0m"
...
Set_cluster(){
  ...
  do
    clear
    tip "存档设置修改后需要重启服务器方能生效！！！<我裂开来./4.21/将第1420行-1425行的if 挪至while后,/4.22/更改1426行0=i为i=0>"
...
Set_token(){
    ...
    then
      ...
      default_token="pds-g^KU_0j0lUQkB^yGI+XSstIr8wXZpYO4tCjEsp0djLwikrrlwgshijnm0="
    fi
    info "当前预设的服务器令牌：\n ${default_token}"
...
cluster_name 世界由千寻创建 房间名称 [NETWORK] read
cluster_description 世界由千寻创建 房间简介 [NETWORK] read
...
cluster_key Chihiro 多世界通信秘钥 [SHARD] readonly
EOF
...
return {
  ...
  desc="你能经受暴食的挑战吗？--世界由千寻创建",
  hideminimap=false,
  id="QUAGMIRE",
  location="quagmire",
  max_playlist_position=999,
  min_playlist_position=0,
  name="暴食：你会做饭吗？--世界由千寻创建",
  numrandom_set_pieces=0,
  override_level_string=false,
  overrides={
    boons="never",
...
checkmodupdate(){
    ...
    then
    	tip "MOD(${new_mod_name})需要更新: 【${cur_mod_ver}----->${new_mod_ver}】。<已更改强制不更新!!!谢谢,需要请更改第2835行false为true>"
    	result="false"
    else
